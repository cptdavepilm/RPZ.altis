//Zombie behavior routines written by Cptdavepilm
waitUntil {!isNil "DZS_initThingsDone"};
...
//Aggro radius
_maxradius = if ({[_x, getPosATL _zombie] call BIS_fnc_inTrigger} count CLY_altmaxaggroradiustriggers == 0) then {CLY_maxaggroradius;} else {CLY_maxaggroradius;};
			...
			if (isNull (_zombie getVariable "victim") && alive _zombie) then
			{
				...
				if (time > _despawncheckwait) then
				{
					...
					if (_despawn) then
					{
						...
						_zombie spawn {sleep 21;_this setDamage 1;};
					};
					_despawncheckwait = time + 10;
				};
			};
