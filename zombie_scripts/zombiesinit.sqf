//Zombie mission init script by Cptdavepilm
...
CLY_zombiehandledamage =
{
	...
	if (isNil "_zombietype") then {_zombietype = "normal"};
		...
		if (_zombietype != "walker") then
		{
			...
			[
				"HandleDamage",
				{
					...
					{
						case "" : {_damage = _olddamage + _newdamage * 0.8;};
						case "head_hit" : {_olddamage + _newdamage * 5;};
						case "body" : {_damage = _olddamage + _newdamage * 0.8;};
						case "legs" : {_damage = _olddamage + _newdamage * 0.8;};
					};
...
//CLY Remove Dead
[45,0] execVM "zombie_scripts\cly_removedead.sqf";
...
//Load the script
CLY_accuracyscript = compile preProcessFile "zombie_scripts\cly_accuracy.sqf";
