...
if (_id in ["give"]) then
{
	...
	titleText [format ["%1 %2",localize "STR_DZS_gaveBandage", name _target], "PLAIN DOWN", 0.3];
	_text = [format ["%1 %2", localize "STR_DZS_receiveBandage",name _caller], "PLAIN DOWN", 0.3];
...
if (_id in ["heal", "healother"]) then
{
	...
	if (_weapontype == 2 || (_weapontype == 0 && 2 in _weapontypes)) then {_anim = "ainvpknlmstpslaywrfldnon_medic";};
	
	if (_id == "healother") then
	{
		_text = [format ["%1 %2", name _caller, localize "STR_DZS_beingBandaged"], "PLAIN DOWN", 0.3];
		CLY_heal_announcepv = [_target, _text];
		publicVariable "CLY_heal_announcepv";
	};
...
while {typeName _id == "SCALAR"} do
{
	...
	while {alive _unit && _unit == player} do
	{
		...
		if (_reset) then
		{
			...
			[
				format ["%1%2", _structure,localize "STR_DZS_Bandagewounds"],
				"zombie_scripts\cly_heal.sqf", 
				"heal",		//Argument
				1.37,		//Priority
			...
			[
				format ["%1%2", _structure, localize "STR_DZS_cancelBandaging"],
				"zombie_scripts\cly_heal.sqf",
				"interrupt",
				1.4,
				true,
				...
				if (alive _x && _x isKindOf "Man") then
				{
					if (isNil {_x getVariable "CLY_heal_healotheraction"}) then
					{
						if (_unit countEnemy [_x] == 0 && _x countEnemy [_unit] == 0) then
						{
							...
							[
								format ["%1%2 %3", _structure, localize "STR_DZS_verbBandage",name _x],
								"zombie_scripts\cly_heal.sqf",
								"healother",
								1.38,
								true,
					...
					if (isNil {_x getVariable "CLY_heal_giveaction"}) then
					{
						if (!isNil {_x getVariable "CLY_heal_bandages"}) then
						{
							if (_unit countEnemy [_x] == 0 && _x countEnemy [_unit] == 0) then
							{
								...
								[
									format ["%1%2 %3", _structure,localize "STR_DZS_giveBandage", name _x],
									"zombie_scripts\cly_heal.sqf",
									"give",
									1.39,
									true,
				...
				else
				{
					if (isNil {_x getVariable "CLY_heal_takeaction"}) then
					{
						if (_x getVariable ["CLY_heal_bandages", 0] > 0) then
						{
							...
							[
								format ["%1%2%3", _structure,localize "STR_DZS_takeBandage", if (_x getVariable ["CLY_heal_bandages", 1] > 1) then {"s"} else {""}], 
								"zombie_scripts\cly_heal.sqf", 
								"take", 
								1.4, 
								true, 
	...
	if (!alive _unit && _unit getVariable ["CLY_heal_bandages", 0] > 0) then
	{
		_action =
		[
			format ["%1%2%3", _structure,localize "STR_DZS_takebandage", if (_unit getVariable "CLY_heal_bandages" > 1) then {"s"} else {""}], 
			"zombie_scripts\cly_heal.sqf", 
