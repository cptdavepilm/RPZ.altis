DZS_fn_spawnTriggers = {
	...
	while {_w} do {
		...
		if (!surfaceIsWater _pos || _set) then {
			...
			if (CVG_Horde) then {
				_trig setTriggerStatements ["true", "0=[thisTrigger,CVG_Zdensity] execVM 'zombie_scripts\cly_z_generatorhorde.sqf';", ""];
			} else {
				_trig setTriggerStatements ["true", "0=[thisTrigger,CVG_Zdensity] execVM 'zombie_scripts\cly_z_generator.sqf';", ""];
			};
			if (CVG_Debug) then {
...
DZS_fn_weather = {
	...
	if (CVG_fog == 1) then {[player,100,11,30,3,7,-0.3,0.1,0.5,1,1,1,13,12,15,false,2,2.1,0.1,1,1,0,0,24] execFSM "mission\Fog.fsm"};
};
				...
				for "_i" from 0 to (_num) do
				{
					...
					if (!isNil "_class") then {
						...
						if (typeName _pos == "ARRAY") then {
							[_zombie,"passive",objNull,false,0,[_pos select 0, _pos select 1, _pos select 2],true] exec "zombie_scripts\cly_z_init.sqs";
				...
				if (_num < 30) then {
					...
					if (count _rds > 0) then {
						...
						if (!isNil "_class" && not (isNil "_pos")) then {
							...
							_zSpawnNum = _zSpawnNum + 1;
							_build setVariable ["Infested",false,true];
							[_zombie,"passive",objNull,false,0,[_pos select 0, _pos select 1],true] exec "zombie_scripts\cly_z_init.sqs";
